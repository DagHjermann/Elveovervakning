...
nc_rivers <- st_read(
  ...
  query = "SELECT * FROM elvis_hovedelv WHERE STRAHLER >= 7")  # only big rivers; saves a little time
#  UTM zone 33

# Extract coordinates and Transform from UTM33 to UTM32
df_rivers <- st_coordinates(nc_rivers) %>% 
  as.data.frame() %>%
  add_transformed_coord(
    name_x = "X", name_y = "Y", 
    from = "utm", from_zone = 33,
...
df_rivers <- fix_river_errors(df_rivers) %>% 
  filter(y < 7240000)    # Not Nord-Norge. Use plotly (above) to set limit
...
gg2 <- gg_back +
  geom_path(data = df_rivers %>% filter(y < 7240000), 
...
if (FALSE){
  ...
  # Select large rivers
  nc_lakes_sel <- nc_lakes %>%
    filter(AREA >= 97.9E6)
  
  # Extract coordinates and Transform from UTM33 to UTM32
  df_lakes_sel <- st_coordinates(nc_lakes_sel) %>% 
    as.data.frame() %>%
    add_transformed_coord(
      name_x = "X", name_y = "Y", 
      from = "utm", from_zone = 33,
  ...
  gg_back +
    geom_path(data = df_lakes_sel, aes(group = group), col = "red") 
  ...
  #
  gg <- ggplot() +
    geom_path(data = df_lakes_sel, aes(x, y, group = group), col = "red")   
  plotly::ggplotly(gg)
