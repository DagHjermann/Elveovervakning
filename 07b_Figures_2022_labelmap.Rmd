...
south_limit <- min(df_counties_full$y) - 300000 # 300 km S and N of li,its
...
### e. Add svg labels to map  

* Make a ggplot without labels, save as svg, and add labels from svg above  
    - Open svg  
    - Open `07b_labelmap_adj02.svg` (see previous chunk)  
    - Select all labels by choosing one, then Edit:Select Same:Object Type   
    - Edit:Copy, then chahe to map without labels and select Edit:Paste in place  
...
* Note: was part 7 in last year's version
...
### b. Define inset box 1        

* Note: last year's code (see 6a1 in that script) used a different approach, using functions get_center, addRectangles2 and make_box_code    
    - also operated in lat/long, not UTM   
    
```{r, fig.width=8, fig.height=10}

# Plot all points (in UTM coordinates)
gg <- ggplot(data = df_stations_all, aes(x, y, shape = Stasjonstype)) +
  geom_point() +
  scale_x_continuous(breaks = seq(400000, 800000, by = 50000), 
                     minor_breaks = seq(400000, 800000, by = 10000)) +
  scale_y_continuous(breaks = seq(6E6, 8E6, by = 50000), 
                     minor_breaks = seq(6E6, 8E6, by = 10000)) +
  theme(panel.grid.major = element_line(size = 2))

# Use this to find corners x_lim and y_lim:
# gg

x_lim <- c(551000, 630000) 
y_lim <- c(7E6, 7.051E6) 
...
gg + 
  annotate("rect", 
           ...
           ymin = y_lim[1], ymax = y_lim[2], fill = NA, colour = "red"
           )

```

### c. Define box_corners and box_polygons  
