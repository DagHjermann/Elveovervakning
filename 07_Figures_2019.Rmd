...
gg_trond <- ggplot(data = df_stations_incl_long[sel,], aes(Long, Lat, label = Shortname)) + 
  ...
  geom_text_repel(data = df1,
                  nudge_x = max(df1$Long) + 0.1 - df1$Long,
                  point.padding = 0.1, 
                  direction = "y", 
                  size = labelsize,
                  hjust = 0
  ...
  geom_text_repel(data = df2,
                  nudge_x = max(df2$Long) + 0.1 - df2$Long,
                  point.padding = 0.1, 
                  direction = "y", 
                  size = labelsize,
                  hjust = 0
...
#if (FALSE){
if (TRUE){
  sel <- df_stations_incl_long$Lat > subset(df_stations_incl_long, substr(Shortname,1,2)=="43")$Lat 
