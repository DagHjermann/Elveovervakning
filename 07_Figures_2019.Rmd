...
gg_nordland <- ggplot(data = df_stations_incl_long[sel,], aes(Long, Lat, Long, Lat, label = Shortname)) +
    geom_blank() +
    annotation_map(map_norway_h, aes(long, lat), fill = "navajowhite") +
    geom_point(aes(Long, Lat, fill = Stasjonstype, shape = Stasjonstype), size = rel(pointsize_z2)) +
    scale_fill_manual(values = cols) +
    scale_shape_manual(values = shapes) +
...
fn <- sprintf("Figures/07_Map_for_methods_chapter_n_with_insets_%s.png", size_label_for_file)
cowplot::ggsave(fn, gg_with_insets, width = 10.8, height = 8, dpi = 500)
...
if (save_plots)  {
  ...
  fn <- sprintf("Figures/07_Map_for_methods_chapter_n_with_insets2_%s.png", size_label_for_file)
