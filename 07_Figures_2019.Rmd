...
Second round of maps/figures:    
* Map for methods (points = type of station) w/o county borders - 3a-c (3c5 used in report)
* Map for methods (points = type of station) with county borders - 3d-h
* Map for results (points = pie) w/o county borders - 4g-h (4h2 used in report)
* Map for results (points = pie) with county borders - 5
...
gg_entire <- gg +
  geom_text_repel(data = df_stations_incl_long %>% filter(Lat < 62),
...
if (save_plots)  {
  ggsave("Figures/07_Map_for_methods_chapter_n_entire1.png", gg_entire, width = 8, height = 6, dpi = 500) 
...
sel <- with(df_stations_incl_long, 
...
# 
if (FALSE){
  ggplot(data = df_stations_incl_long[sel,], aes(Long, Lat, Long, Lat, label = Shortname)) +
    geom_blank() +
    annotation_map(map_norway_h, aes(long, lat), fill = "navajowhite") +
    geom_point(aes(Long, Lat, fill = Stasjonstype, shape = Stasjonstype)) +
    scale_fill_manual(values = cols) +
    scale_shape_manual(values = shapes) +
    geom_text_repel(data = df_stations_incl_long[sel,],
                    point.padding = 0.1,
...
p1 <- subset(df_stations_incl_long, substr(Shortname,1,2) == "29")  # main division (W and east)
p2 <- subset(df_stations_incl_long, substr(Shortname,1,2) == "34")  # divides the E labels in two

df1 <- subset(df_stations_incl_long[sel,], Long > p1$Long & Lat > p2$Lat)      # NE points
df2 <- subset(df_stations_incl_long[sel,], Long > p1$Long & Lat <= p2$Lat)     # SE points
df2 <- bind_rows(df2,
                 subset(df_stations_incl_long[sel,], Shortname == "Gruve.F7")) # SE points, one extra
df3 <- subset(df_stations_incl_long[sel,], Long <= p1$Long)                    # W points


#p1

gg_trond <- ggplot(data = df_stations_incl_long[sel,], aes(Long, Lat, label = Shortname)) + 
  geom_blank() +
  annotation_map(map_norway_h, aes(long, lat), fill = "navajowhite") +
  geom_point(aes(Long, Lat, fill = Stasjonstype, shape = Stasjonstype), size = rel(1.7)) +
  scale_fill_manual(values = cols) +
  scale_shape_manual(values = shapes) +
...
if (save_plots)  {
  ggsave("Figures/07_Map_for_methods_chapter_n_z1.png", gg_trond, width = 8, height = 6, dpi = 500) 
...
sel <- with(df_stations_incl_long, Lat > 65)

# Aspect ratio
aspectratio <- 111500/longitude.length(mean(range(df_stations_incl_long$Lat[sel])))

labelsize <- 4

gg_nordland <- ggplot(data = df_stations_incl_long[sel,], aes(Long, Lat, Long, Lat, label = Shortname)) +
    geom_blank() +
    annotation_map(map_norway_h, aes(long, lat), fill = "navajowhite") +
    geom_point(aes(Long, Lat, fill = Stasjonstype, shape = Stasjonstype)) +
    scale_fill_manual(values = cols) +
    scale_shape_manual(values = shapes) +
    geom_text_repel(aes(Long + 0.1),  # 0.2: move start point of lines to right
                  point.padding = 0.05,   # Avoid overlap with points
                  nudge_x = 0.4,         # Nudge labels to the East
...
if (save_plots)  {
  ggsave("Figures/07_Map_for_methods_chapter_n_z2.png", gg_nordland, width = 7, height = 6, dpi = 500) 
...
img_trondelag <- magick::image_read("Figures/07_Map_for_methods_chapter_n_z1.png") %>%
  magick::image_transparent(color = "white")

img_nordland <- magick::image_read("Figures/07_Map_for_methods_chapter_n_z2.png") %>%
  magick::image_transparent(color = "white")
...
cowplot::ggsave("Figures/07_Map_for_methods_chapter_n_with_insets.png", 
                gg_with_insets, width = 10.8, height = 8, dpi = 500)

# Show the saved version
# (which is the actual end product) 
magick::image_read("Figures/07_Map_for_methods_chapter_n_with_insets.png") %>% plot()
...
gg_with_insets2 <- cowplot::ggdraw(gg_for_inset) +
  ...
  cowplot::draw_image(img_trondelag, x = 0.3, y = -0.18, width = 0.55, height = 0.75, scale = 1.2)
# gg_with_insets
...
if (save_plots)  {
  ...
  cowplot::ggsave("Figures/07_Map_for_methods_chapter_n_with_insets2.png", 
                  gg_with_insets2, width = 10.8, height = 13, dpi = 500)

  ggsave("Figures/07_Map_for_methods_chapter_n_entire2.png", gg_entire2, width = 8, height = 6, dpi = 500) 
  
}

# 
magick::image_read("Figures/07_Map_for_methods_chapter_n_with_insets2.png") %>% plot()
...
#
if (save_plots)  {
  ggsave("Figures/07_Map_for_methods_chapter_n_ver1.png", gg3_ver1, width = 10, height = 10, dpi = 500) 
  ggsave("Figures/07_Map_for_methods_chapter_n_ver1.svg", gg3_ver1, width = 10, height = 10) 
  ggsave("Figures/07_Map_for_methods_chapter_n_ver2.png", gg3_ver2, width = 10, height = 10, dpi = 500) 
  ggsave("Figures/07_Map_for_methods_chapter_n_ver2.svg", gg3_ver2, width = 10, height = 10) 
  }
...
if (save_plots)
  ggsave("Figures/07_Map_tilstand_03_Trondelag.png", gg_zoom1, width = 11, height = 7, dpi = 500) 
...
if (save_plots)
   ggsave("Figures/07_Map_tilstand_02_Nordland.png", gg_zoom2, width = 11, height = 7, dpi = 500) 
...
img_trondelag <- magick::image_read("Figures/07_Map_tilstand_03_Trondelag.png") %>%
  magick::image_transparent(color = "white")

img_nordland <- magick::image_read("Figures/07_Map_tilstand_02_Nordland.png") %>%
  magick::image_transparent(color = "white")
...
cowplot::ggsave("Figures/07_Map_tilstand_02_entire_with_insets.png", gg2_with_insets, width = 7, height = 8, dpi = 400)
